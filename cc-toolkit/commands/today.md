...
**오늘** Claude Code로 무엇을 했는지 **세션별로** 분석해서 보여줘.

## 실행 단계

1. **오늘 timestamp 범위 계산** (밀리초 단위)
   ```bash
   START_TS=$(($(date -d "today 00:00:00" +%s) * 1000))
   END_TS=$(($(date -d "tomorrow 00:00:00" +%s) * 1000))
   ```

2. **history.jsonl에서 오늘의 프로젝트 추출**
   ```bash
